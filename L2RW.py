...
def adjust_lr(RW_C, i, warmup=True, min_lr=1e-3):
    ...
    lr = lr * (
        (1 + np.cos(np.pi * (i - warmup_steps) / (RW_C.N_steps - warmup_steps))) / 2
        + min_lr
    )
...
class L2GradRW_M(object):
    def __init__(
        ...
        self.optimizer = torch.optim.Adam(
            self.RW.get_para(), **Adam_params, amsgrad=True
        )
        ...
        self.beta_min = (
            beta  # minimal beta value since accept rate takes a while to recover...
        )
    ...
    def sample_train(self, x, T=1, train=True, print_log=False):
        ...
        Loss = (
            -log_a_clip - beta * sldjxx
        )  # standard entropy regulated loss for sampling equilibrium distribution
        ...
        if print_log:
            print(
                "jump {:.3e}, Exp jump{:.3e}, accept {:.3f},log_a {:.3e}, sldjxx {:.3f},beta {:.3e}, epsi {:.3e}".format(
                    L2_jump.mean().item(),
                    L2_jump_expected.mean().item(),
                    a_avg.item(),
                    log_a.mean().item(),
                    sldjxx.mean().item(),
                    self.beta,
                    self.RW.epsi.exp().item(),
                )
            )
        ...
        x[accept_vect] = xp[
            accept_vect
        ].detach()  # burn in samples don't go through MH step.
