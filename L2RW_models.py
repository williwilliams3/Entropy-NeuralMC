...
def gen_mask(Nside, mask_type):
    ...
    if mask_type == "conv":
        ...
        x_mask = np.stack([bi_mask, bi_mask_conj, bi_mask], axis=0).reshape(
            1, 3, Nside, Nside
        )
        x_mask_conj = ~x_mask
    elif mask_type == "MLP":
...
class MeanPoolConv3x3(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channel, out_channel, 3, stride=1, padding=1, bias=False
        )
...
class Conv_SQT(nn.Module):
    def __init__(
        self, in_chan, dim, out_layers, in_layers, mode="SQT", act="relu", N_side=32
    ):
        super().__init__()
        ...
        self.conv1_list = nn.ModuleList(
            [nn.Conv2d(in_stacks * in_chan, dim, 3, 1, 1) for i in range(in_layers)]
        )  # 32x32xdim
        ...
        self.convup2 = nn.ConvTranspose2d(
            2 * dim, dim, 3, 2, 1, output_padding=1
        )  # 32x32xdim
        # condition on step number with different output layer modules..
        self.convup1_list = nn.ModuleList(
            [
                nn.ConvTranspose2d(
                    dim, self.out_stacks * in_chan, 3, 1, 1, output_padding=0
                )
...
class Conv_U_SQT(nn.Module):
    def __init__(
        ...
        self.convup4 = nn.ConvTranspose2d(
            8 * dim, 4 * dim, 3, 2, 1, output_padding=1
        )  # 8x8x4*dim
        self.convup3 = nn.ConvTranspose2d(
            8 * dim, 2 * dim, 3, 2, 1, output_padding=1
        )  # 16x16x2*dim
        self.convup2 = nn.ConvTranspose2d(
            4 * dim, dim, 3, 2, 1, output_padding=1
        )  # 32x32xdim
        ...
        self.convup1_list = nn.ModuleList(
            [
                nn.ConvTranspose2d(
                    2 * dim, self.out_stacks * in_chan, 3, 1, 1, output_padding=0
                )
                for i in range(out_layers)
            ]
        )
...
class MLP_SQT(nn.Module):
    def __init__(self, in_dim, dim, out_layers, in_layers, mode="SQT", act="relu"):
        ...
        self.L1_list = nn.ModuleList(
            [nn.Linear(in_stacks * in_dim, dim) for i in range(in_layers)]
        )
        self.L2 = nn.Linear(dim, dim)
        self.L3 = nn.Linear(dim, dim)
        self.L4 = nn.Linear(dim, dim)
        self.L5_list = nn.ModuleList(
            [nn.Linear(dim, self.out_stacks * in_dim) for i in range(out_layers)]
        )
...
class L2GradRW(nn.Module):
    def __init__(
        ...
        self.net_type = (
            net_type  # MLP for toy tasks and unstructrued tasks, conv for images
        )
        self.grad_mode = (
            grad_mode  # if to use gradient of data distribution, 'no', 'x', 'R'
        )
        self.bias = bias
        ...
        if net_type == "MLP":
            self.v_size = in_chan
            self.netV = MLP_SQT(
                in_chan, dim, N_steps + 1, N_steps + 1, mode="SQT_RW", act=act
            )
            if grad_mode == "R":
            ...
            v_mask, v_mask_conj = torch.cat(v_mask_list, dim=0), torch.cat(
                v_mask_conj_list, dim=0
            )
            ...
            self.netV = Conv_SQT(
                in_chan, dim, N_steps + 1, N_steps + 1, mode="SQT_RW", N_side=N_side
            )
            if grad_mode == "R":
                self.netR = Conv_SQT(
                    in_chan, dim, N_steps, N_steps, mode="R", act=act, N_side=N_side
                )
    ...
    def forward(self, x, v, reverse=False, TE=1, grad=True):
        ...
        if not reverse:
            for i in range(self.N_steps):
                ...
                v = v_mask * v + v_mask_conj * (
                    v * S.exp() - epsi * (gradE * Q.exp() + self.bias * T)
                )
            ...
            if self.grad_mode == "x":
                ...
                v -= (
                    self.epsi.exp() * self.grad_E(x, T=TE, grad=False) * Q.exp() / 2 + T
                )
            ...
            for i in range(self.N_steps, 0, -1):
                ...
                S, Q, T = self.netV([x, v_mask_conj * v, gradE], i, i)
                v = v_mask_conj * v + v_mask * (
                    (v + epsi * (gradE * Q.exp() + self.bias * T)) * (-S).exp()
                )
                sldj += (v_mask * (-S)).flatten(1).sum(1)
    ...
    def sample(self, x, T=1, grad=True):
        ...
        log_a = (
            Ee - Eep + Ex - Exp + sldjf + sldjb
        )  # here we can directly calculate log_a the accept probability
...
class CovMALA(nn.Module):
    def __init__(
        self, netE, in_chan, epsi=0.05, net_type="MLP", full_cov=True, N_side=32
    ):
        super().__init__()
        ...
        self.dim = dim
        if full_cov:
            L = torch.diag(torch.zeros(dim)).view(
                1, dim, dim
            )  # I think it will be advantageous to parameterize diagonal element of L in log space?
            mask = torch.from_numpy(
                np.triu(np.ones((dim, dim))) == 1
            ).bitwise_not_()  # mask for lower trangular part
    ...
    def sample(self, x, T=1, grad=True):
        ...
        gradE = self.reshape_x(
            self.grad_E(self.reshape_x(x, flatten=False), T=T, grad=False), flatten=True
        )
        ...
        gradEp = self.reshape_x(
            self.grad_E(self.reshape_x(xp, flatten=False), T=T, grad=False),
            flatten=True,
        )

        if self.full_cov:
            z0p = (self.epsi / 2) * torch.matmul(
                self.get_L(transpose=True), gradE + gradEp
            ) - z0
        else:
    ...
    def get_L(self, transpose=False):
        L = self.L * self.mask + torch.diag(self.L_diag.exp()).view(
            1, self.dim, self.dim
        )
        if transpose:
            L = L.transpose_(1, 2)
